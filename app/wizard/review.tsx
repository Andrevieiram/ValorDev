...
export default function WizardReviewScreen() {
  ...
  const handleGenerateEstimate = async () => {
    try {
      ...
      // Chamar o backend para calcular pricing e risco
      const pricingResponse = await proposalsApi.calculate(wizardData);

      // Guardar o resultado no store
      const pricingResult: PricingResult = {
        minimum: pricingResponse.minimumPrice,
        recommended: pricingResponse.recommendedPrice,
        premium: pricingResponse.premiumPrice,
        confidence: pricingResponse.confidence,
        riskLevel: pricingResponse.riskLevel,
        riskScore: pricingResponse.riskScore,
        breakdown: pricingResponse.breakdown || [],
        riskFactors: pricingResponse.riskFactors || [],
      };
      ...
      Alert.alert('Erro', 'Não foi possível calcular a estimativa. Tente novamente.', [
        { text: 'OK' },
      ]);
    } finally {
      setIsLoading(false);
        ...
        <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
          <View className="gap-6 pb-8">
            ...
            <Card variant="outlined" className="gap-4">
              <Text className="text-sm font-semibold text-primary">1. Perfil do Desenvolvedor</Text>
            ...
            <Card variant="outlined" className="gap-4">
              <Text className="text-sm font-semibold text-primary">2. Contexto do Projeto</Text>
              <View className="gap-3">
                <View className="flex-row justify-between">
            ...
            {/* Client Card */}
            <Card variant="outlined" className="gap-4">
              <Text className="text-sm font-semibold text-primary">3. Perfil do Cliente</Text>
              <View className="gap-3">
                <View className="flex-row justify-between">
            ...
            <View className="gap-3 pt-2">
              <Button
                size="lg"
                label={isLoading ? 'Calculando...' : 'Gerar estimativa comercial'}
